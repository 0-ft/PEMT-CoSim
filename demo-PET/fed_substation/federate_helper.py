...
class FEDERATE_HELPER:
    def __init__(self, configfile, helics_config_path, metrics_root, hour_stop):
        ...
        self.auction_meta = {'clearing_price': {'units': 'USD', 'index': 0},
        ...
        self.prosumer_meta = {'bid_price': {'units': 'USD', 'index': 0}, 'bid_quantity': {'units': unit, 'index': 1},
                              'hvac_needed': {'units': unit, 'index': 2}, 'role': {'units': unit, 'index': 3}}
    ...
    def register_pubssubs(self):
        ...
        for house_name, val in self.housesInfo_dict.items():
            ...
            self.pubsMtrMode[house_name] = helics.helicsFederateGetPublication(self.hFed,
                                                                               billMeterPubTopic + '/bill_mode')
            self.pubsMtrPrice[house_name] = helics.helicsFederateGetPublication(self.hFed, billMeterPubTopic + '/price')
            self.pubsMtrMonthly[house_name] = helics.helicsFederateGetPublication(self.hFed,
                                                                                  billMeterPubTopic + '/monthly_fee')
            self.subsVolt[house_name] = helics.helicsFederateGetSubscription(self.hFed,
                                                                             billMeterSubTopic + '#measured_voltage_1')
            self.subsMtrPower[house_name] = helics.helicsFederateGetSubscription(self.hFed,
                                                                                 billMeterSubTopic + '#measured_power')
            self.subsMtrDemand[house_name] = helics.helicsFederateGetSubscription(self.hFed,
                                                                                  billMeterSubTopic + '#measured_demand')
            self.subsHousePower[house_name] = helics.helicsFederateGetSubscription(self.hFed,
                                                                                   houseMeterSubTopic + '#measured_power')

            self.subsTemp[house_name] = helics.helicsFederateGetSubscription(self.hFed,
                                                                             houseSubTopic + '#air_temperature')
            self.subsState[house_name] = helics.helicsFederateGetSubscription(self.hFed, houseSubTopic + '#power_state')
            self.subsHVACLoad[house_name] = helics.helicsFederateGetSubscription(self.hFed,
                                                                                 houseSubTopic + '#hvac_load')
            self.subsHouseLoad[house_name] = helics.helicsFederateGetSubscription(self.hFed,
                                                                                  houseSubTopic + '#total_load')
            self.pubsHeatingSetpoint[house_name] = helics.helicsFederateGetPublication(self.hFed,
                                                                                       hvacPubTopic + '/heating_setpoint')
            self.pubsCoolingSetpoint[house_name] = helics.helicsFederateGetPublication(self.hFed,
                                                                                       hvacPubTopic + '/cooling_setpoint')
            self.pubsDeadband[house_name] = helics.helicsFederateGetPublication(self.hFed,
                                                                                hvacPubTopic + '/thermostat_deadband')
            self.pubsThermostatState[house_name] = helics.helicsFederateGetPublication(self.hFed,
                                                                                       hvacPubTopic + '/thermostat_mode')  # new added by Yuanliang
            ...
            if val['PV'] != None:
                ...
                self.subsSolarPower[house_name] = helics.helicsFederateGetSubscription(self.hFed,
            ...
            if val['battery'] != None:
                ...
                self.subsBattPower[house_name] = helics.helicsFederateGetSubscription(self.hFed,
                                                                                      battMeterSubTopic + '#measured_power')
                self.subsBattSoC[house_name] = helics.helicsFederateGetSubscription(self.hFed,
                                                                                    battSubTopic + '#state_of_charge')

                battery_inv_name = val['battery']
                battCtlPubTopic = self.fedName + '/' + battery_inv_name
                self.pubsCharge_on_threshold[house_name] = helics.helicsFederateGetPublication(self.hFed,
                                                                                               battCtlPubTopic + '/charge_on_threshold')
                self.pubsCharge_off_threshold[house_name] = helics.helicsFederateGetPublication(self.hFed,
                                                                                                battCtlPubTopic + '/charge_off_threshold')
                self.pubsDischarge_on_threshold[house_name] = helics.helicsFederateGetPublication(self.hFed,
                                                                                                  battCtlPubTopic + '/discharge_on_threshold')
                self.pubsDischarge_off_threshold[house_name] = helics.helicsFederateGetPublication(self.hFed,
                                                                                                   battCtlPubTopic + '/discharge_off_threshold')
        ...
        for i, vpp_name in enumerate(self.vpp_name_list):
            ...
            self.subsVPPMtrPower[vpp_name] = helics.helicsFederateGetSubscription(self.hFed,
                                                                                  vppSubTopic + '#measured_power')
